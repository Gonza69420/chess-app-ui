...
public class InfiniteMoveValidator implements Validator {
    @Override
    public boolean validate(int x1, int y1, int x2, int y2, Board board, Color color, WinCondition winCondition) {
        ...
        int maxLength = board.getXLength();
        if (board.getXLength() < board.getYLength()){
            maxLength = board.getYLength();
        }
        List<InfiniteMove> moves = piece.getMovementBehaviour().getInfiniteMoves();
        for (InfiniteMove move : moves) {
            int possibleX = x1;
            int possibleY = y1;
            for (int i = 0; i < maxLength ; i++) {
                possibleX += move.getPlusX();
                possibleY = possibleY + move.getPlusY();
                if (possibleX >= board.getXLength() || possibleX < 0){
                    break;
                }
                if (possibleY >= board.getYLength() || possibleY < 0){
                    break;
                }
                if (possibleX == x2 && possibleY == y2){
